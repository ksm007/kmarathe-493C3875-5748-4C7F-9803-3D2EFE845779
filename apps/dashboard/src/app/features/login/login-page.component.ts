...
@Component({
  ...
  template: `
    <section
      class="flex min-h-screen items-center justify-center bg-surface-container-low px-4 py-10"
    >
      <main
        class="relative flex w-full max-w-[440px] flex-col gap-lg overflow-hidden rounded-xl border border-outline-variant/40 bg-surface-container-lowest p-xl shadow-soft"
      >
        <div
        ...
        <header class="mt-sm flex flex-col items-center text-center">
          <div
            class="mb-md flex h-14 w-14 items-center justify-center rounded-xl bg-primary-container text-on-primary-container shadow-sm"
          >
            <span class="material-symbols-outlined fill text-[32px]"
              >task_alt</span
            >
          </div>
        ...
        <form
          class="mt-sm flex flex-col gap-md"
          [formGroup]="form"
          (ngSubmit)="submit()"
        >
        ...
        <div
          class="mt-sm flex flex-col gap-md border-t border-outline-variant/30 pt-lg"
        >
          <button
            class="flex w-full items-center justify-center gap-2 rounded-xl border border-outline-variant bg-surface px-4 py-3 font-label-md text-label-md text-on-surface transition-colors hover:bg-surface-container-low"
            type="button"
          >
            <svg
              class="h-5 w-5"
          ...
          <p
            class="text-center font-label-sm text-label-sm text-on-surface-variant"
          >
            Having trouble accessing your account?
            <button class="text-primary hover:underline" type="button">
              Contact IT Support
            </button>
          </p>
        </div>
...
export class LoginPageComponent {
  ...
  submit() {
    ...
    this.store.dispatch(
      AuthActions.loginRequested({ credentials: this.form.getRawValue() }),
    );
  }
}
